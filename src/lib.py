...
class FileServer(chunk_pb2_grpc.FileServerServicer):
    def __init__(self):
        ...
        file_server = grpc.server(futures.ThreadPoolExecutor(max_workers=1))
        chunk_pb2_grpc.add_FileServerServicer_to_server(FileServer(), file_server)

    def receive_file(self, request_iterator, out_path="/tmp/file"):
        save_chunks_to_file(request_iterator, out_path)
